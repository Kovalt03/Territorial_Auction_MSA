...
|---|---|---|---|---|
| POST | `/internal/wallets/bid-escrow` | `{bidderId, bidAmount, previousBidderId?, previousAmount?}` | `{bidderNickname}` | 404 USER_NOT_FOUND · 409 INSUFFICIENT_AP |
| POST | `/internal/wallets/consume-locked` | `{winnerId, finalPrice, auctionId}` | 200 | 404 USER_NOT_FOUND |
| POST | `/internal/wallets/refund-locked` | `{bidderId, amount}` | 200 | 404 USER_NOT_FOUND |
...
응답 DTO는 모놀리식 admin 응답과 **필드명이 일치**해 그대로 역직렬화된다(스냅샷 필드만으로 구성). force-cancel은 입찰자 잠금 AP 환불(→ 모놀리식 `refund-locked`) + 영토 재경매 예약(→ `release`) + 종료.
