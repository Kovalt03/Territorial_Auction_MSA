...
class ItemDailyLimitTest {
    ...
    @DisplayName("한도 있는 아이템 구매 — 한도 조회 전에 유저 단위 advisory lock을 먼저 잡는다")
    @Test
    void purchase_limitedItem_acquiresLockBeforeDailyLimitCheck() {
        when(itemRepository.findById(1L)).thenReturn(Optional.of(limitedItem(5)));
        when(itemPurchaseRepository.sumTodayQuantity(eq(USER_ID), eq(1L), any(LocalDateTime.class)))
                .thenReturn(4);
        ...
        InOrder order = inOrder(itemPurchaseRepository);
        order.verify(itemPurchaseRepository).acquireDailyLimitLock(USER_ID);
        order.verify(itemPurchaseRepository)
                .sumTodayQuantity(eq(USER_ID), eq(1L), any(LocalDateTime.class));
    }

    @DisplayName("한도 없는 아이템 구매 — advisory lock을 잡지 않는다")
    @Test
    void purchase_unlimitedItem_skipsLock() {
        when(itemRepository.findById(2L)).thenReturn(Optional.of(unlimitedItem()));
        when(userItemRepository.findByUserIdAndItem_Id(USER_ID, 2L)).thenReturn(Optional.empty());
        when(userItemRepository.save(any()))
                .thenReturn(UserItem.builder().userId(USER_ID).quantity(1).build());
        when(walletClient.spend(eq(USER_ID), anyInt(), anyString()))
                .thenReturn(new WalletClient.WalletSnapshot(900, 0));

        itemService.purchaseItem(USER_ID, new PurchaseItemRequest(2L, 1, null));

        verify(itemPurchaseRepository, never()).acquireDailyLimitLock(any());
        verify(walletClient).spend(eq(USER_ID), anyInt(), anyString());
    }
}
