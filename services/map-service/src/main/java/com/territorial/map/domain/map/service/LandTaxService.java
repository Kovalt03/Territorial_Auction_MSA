...
import com.territorial.map.domain.map.LandTaxPolicy;
...
public class LandTaxService {
    ...
    private void applyTaxOrEvict(Long userId, int territoryCount, int taxAmount) {
        ...
        if (paid) {
            ...
            notificationClient.sendNotification(
                    userId, "TAX_CHARGED", "토지세 " + taxAmount + " GP가 정상 납부되었습니다.");
            log.info(
    ...
    private void enforceSelectiveEviction(Long userId, int taxAmount) {
        ...
        notificationClient.sendNotification(
                userId, "TAX_EVICTION", evictedCount + "개 영토가 강제 경매 전환됐습니다.");
        log.info(
