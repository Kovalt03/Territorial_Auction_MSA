...
public interface TerritoryRepository extends JpaRepository<Territory, Long> {
    ...
    long countByOwnerIdIn(List<Long> ownerIds);
    ...
    @Query("SELECT t FROM Territory t JOIN FETCH t.continent JOIN FETCH t.grade WHERE t.id = :id")
    ...
    @Query(
            "SELECT t FROM Territory t JOIN FETCH t.grade "
    ...
    @Query(
            "SELECT t.ownerId, COUNT(t) FROM Territory t WHERE t.ownerId IN :ownerIds AND t.status = :status GROUP BY t.ownerId")
    ...
    @Query(
            value =
                    "SELECT t FROM Territory t JOIN FETCH t.continent JOIN FETCH t.grade where t.ownerId = :userId",
            countQuery = "SELECT COUNT(t) FROM Territory t WHERE t.ownerId = :userId")
    ...
    @Query(
            "SELECT DISTINCT t.ownerId FROM Territory t WHERE t.ownerId IS NOT NULL AND t.status = :status")
    List<Long> findAllDistinctOwnerIds(@Param("status") Territory.TerritoryStatus status);

    List<Territory> findByOwnerId(Long ownerId);

    @Query("SELECT t FROM Territory t WHERE t.ownerId = :userId AND t.status = :status")
