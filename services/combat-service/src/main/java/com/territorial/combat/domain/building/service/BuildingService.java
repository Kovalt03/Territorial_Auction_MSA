...
import com.territorial.combat.domain.building.ZonePolicy;
...
import com.territorial.combat.domain.building.dto.InventoryResponse.InventoryItem;
...
import com.territorial.combat.domain.building.dto.TerritoryBuildingResponse.BuildingInfo;
...
@Service
@ConditionalOnBean({
    TerritoryContextPort.class,
    WalletPort.class,
    SeasonBenefitPort.class,
    BuildingNotificationPort.class
})
...
public class BuildingService {
    ...
    private final com.territorial.combat.domain.building.repository.BuildingLevelSpecRepository
    ...
    private final SeasonBenefitPort seasonBenefitPort;
    private final BuildingNotificationPort notificationPort;
    private final com.territorial.combat.domain.building.repository.BuildingCastleLimitRepository
            buildingCastleLimitRepository;
    private final BuildingBalanceProperties balanceProperties;
    ...
    public TerritoryBuildingResponse findTerritoryBuildings(Long territoryId) {
        findTerritoryOrThrow(territoryId);
    ...
    public PlaceBuildingResponse placeOnTerritory(
        ...
        TerritoryContext territory = findTerritoryOrThrow(territoryId);
        ...
        int gridSize = territory.gridSize();
        int zone = calculateTerritoryZone(request.posX(), request.posY(), territory);
        validatePosition(existing, buildingType, request.posX(), request.posY(), gridSize);
        validateZoneFootprint(
                buildingType,
                request.posX(),
                request.posY(),
                (x, y) -> calculateTerritoryZone(x, y, territory));
        ...
        int gpRemaining = chargeTerritoryGp(territory.territoryId(), buildingType.getBaseCostGp());
    ...
    public RushConstructionResponse rushConstruction(Long userId, Long buildingId) {
        ...
        finishConstruction(building);
        var wallet = walletPort.spend(userId, apCost, "RUSH:" + userId + ":" + buildingId);
    ...
    private void applyLevelMaxHp(BuildingInstance building) {
        int maxHp =
                buildingLevelSpecRepository
                        ...
                        .map(
                                com.territorial.combat.domain.building.entity.BuildingLevelSpec
                                        ::getMaxHp)
    ...
    private void recenterIslandBuildings(HomeIsland island, int oldGridSize) {
        ...
        for (BuildingInstance building : buildings) {
            ...
            building.store(island.getUserId());
            storedCount++;
        }

        if (storedCount == 0) return;
        log.info("섬 확장으로 건물 보관 처리. islandId={}, count={}", island.getId(), storedCount);
        notificationPort.notifyIslandExpanded(island.getUserId(), storedCount);
    }
    ...
    private int applyBuildTimeReduction(Long userId, int seconds) {
        ...
        int reductionPct = seasonBenefitPort.findActiveBenefit(userId).buildTimeReductionPct();
    ...
    public RepairAllResponse repairAll(Long userId, String locationType, Long locationId) {
        ...
        int gpRemaining = -1;
        for (BuildingInstance building : buildings) {
            Long ownerId = resolveBuildingOwnerId(building);
    ...
    private int repairGpPerHp() {
        return balanceProperties.repairGpPerHp();
    }
    ...
    public IslandResponse getIsland(Long userId) {
        ...
        com.territorial.combat.domain.building.BuildingLevelSpecResolver resolver =
                com.territorial.combat.domain.building.BuildingLevelSpecResolver.of(
    ...
    public PlaceFromInventoryResponse placeFromInventory(
        ...
        TerritoryContext territory = findTerritoryOrThrow(request.territoryId());
        validateTerritoryOwner(territory, userId);
        ...
        stored.placeOnTerritory(territory.territoryId(), request.posX(), request.posY(), zone);
    ...
    private int resolveBuilderCount(Long userId) {
        int extraBuilders = seasonBenefitPort.findActiveBenefit(userId).extraBuilders();
    ...
    private void validateBuilderAvailable(Long userId) {
        long buildingNow =
                buildingInstanceRepository.countUnderConstructionByOwnerId(
                        userId,
                        territoryContextPort.findOwnedTerritoryIds(userId),
                        LocalDateTime.now());
    ...
    private TerritoryContext findTerritoryOrThrow(Long territoryId) {
    ...
    private int chargeBuildingLocationGp(BuildingInstance building, int cost) {
        if (building.getTerritoryId() != null) {
            return chargeTerritoryGp(building.getTerritoryId(), cost);
        }
    ...
    private Long resolveBuildingOwnerId(BuildingInstance building) {
        if (building.getTerritoryId() != null) {
            return findTerritoryOrThrow(building.getTerritoryId()).ownerId();
        }
    ...
    private int calculateTerritoryZone(int posX, int posY, TerritoryContext territory) {
        return ZonePolicy.calculateZone(
                posX, posY, territory.gridSize(), territory.zone1Radius(), territory.zone2Radius());
    }
    ...
    private int resolveZone(BuildingInstance building, int posX, int posY) {
        ...
        return calculateTerritoryZone(posX, posY, findTerritoryOrThrow(building.getTerritoryId()));
    }
    ...
    private void validateBuildingLimitOnTerritory(
            BuildingType buildingType, TerritoryContext territory) {
        if (buildingType.isCastle()) return;
        int castleLevel =
                buildingInstanceRepository
                        .findCastleLevelByTerritoryId(territory.territoryId())
                        .orElse(1);
        buildingCastleLimitRepository
                .findByBuildingType_IdAndCastleLevel(buildingType.getId(), castleLevel)
                .ifPresent(
                        limit -> {
                            long placed =
                                    buildingInstanceRepository.countByTerritoryIdAndBuildingTypeId(
                                            territory.territoryId(), buildingType.getId());
                            if (placed >= limit.getMaxCount()) {
                                throw new CustomException(ErrorCode.BUILDING_LIMIT_EXCEEDED);
                            }
                        });
    ...
    private void validateSingleCastleOnTerritory(
            BuildingType buildingType, TerritoryContext territory) {
        if (!buildingType.isCastle()) return;
        if (buildingInstanceRepository.existsCastleOnTerritory(territory.territoryId())) {
            throw new CustomException(ErrorCode.CASTLE_ALREADY_EXISTS);
        }
    }
    ...
    private List<BuildingInstance> findExistingBuildings(BuildingInstance building) {
        if (building.getTerritoryId() != null) {
            return buildingInstanceRepository.findByTerritoryId(building.getTerritoryId());
        }
        if (building.getIsland() != null) {
    ...
    private int resolveGridSize(BuildingInstance building) {
        if (building.getTerritoryId() != null) {
            return findTerritoryOrThrow(building.getTerritoryId()).gridSize();
        }
        if (building.getIsland() != null) {
