...
/**
 * 실시간 허브. map-service가 발행하는 map.update(Redis pub/sub)를 구독해 클라이언트에 STOMP push한다. 경매 낙찰에 의한 OCCUPIED
 * 브로드캐스트는 AuctionRealtimeSubscriber가 담당하고, 여기서는 공성 인계·점유 만료(IDLE)를 relay한다.
 */
@Slf4j
@Component
