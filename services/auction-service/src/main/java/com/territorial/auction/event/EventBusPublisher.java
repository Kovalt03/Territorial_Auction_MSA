 ...
 * auction_outbox_events에 기록(도메인 변경과 원자적) → {@link AuctionOutboxPublisher} 릴레이가 Kafka로 드레인.
 * 따라서 Kafka 장애가 입찰/정산 요청 경로를 실패시키지 않고, 이벤트도 유실 없이 복구 후 전달된다.
 *
 * <p>실시간(WebSocket) 소비자용 Redis pub/sub는 커밋 후 best-effort로 발행한다(저지연, 유실 허용). 반드시
 * <b>활성 트랜잭션 안에서</b> 호출해야 아웃박스 기록이 원자적으로 커밋된다.
 */
@Slf4j
@Component
