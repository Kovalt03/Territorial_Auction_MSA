...
/** 트랜잭셔널 아웃박스 드레인 — 미발행 auction 이벤트를 Kafka(auction-events)로 발행. 전송 성공 시에만 published 표시. */
@Slf4j
@Component
@RequiredArgsConstructor
public class AuctionOutboxPublisher {

    public static final String TOPIC = "auction-events";
    // 이벤트 종류(auction.bid/opened/closed/settled)는 Kafka 헤더로 실어 소비자가 분기한다.
    ...
    public void publishPending() {
        for (AuctionOutboxEvent event :
            ...
            try {
                ...
                log.warn(
                        "Auction outbox 발행 실패. eventId={}, topic={}",
                        event.getId(),
                        event.getTopic(),
                        e);
                return; // 순서 보존을 위해 실패 지점에서 중단, 다음 주기 재시도
            }
        }
    }
}
