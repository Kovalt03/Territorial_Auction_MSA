...
/**
 * season → notification-service 알림 발행. 저장·실시간 배지는 notification-service가 소유하며, season은
 * notification-events(Kafka)로 요청만 넘긴다.
 */
@Slf4j
@Component
            ...
            String payload =
                    objectMapper.writeValueAsString(
                            new NotificationRequested(
                                    java.util.UUID.randomUUID().toString(), userId, type, message));
