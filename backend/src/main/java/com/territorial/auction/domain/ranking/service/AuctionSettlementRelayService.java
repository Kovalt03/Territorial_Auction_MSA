...
/**
 * auction-service의 Redis auction.settled 이벤트를 모놀리식 인프로세스 Spring 이벤트로 중계한다. 랭킹(경매 소비·영토 보유)
 * 리스너가 @TransactionalEventListener(AFTER_COMMIT)라 활성 트랜잭션 안에서 발행돼야 하므로 @Transactional로 감싼다. 시즌
 * XP·미션은 season-service로 위임(game-events)하고, 랭킹 시즌 귀속은 활성 시즌을 조회해 채운다.
 */
@Service
